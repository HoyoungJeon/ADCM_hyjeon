...
# 0부터 π까지 100단계로 나눈 반원 각도
theta = np.linspace(0, np.pi, 100)

# frames_random 형식에 맞춰, 매 프레임마다 한 점의 [x, y] 좌표를 담은 리스트 생성
frames_random = [
...
# CTRA6: θ(T_total) = 2π 가 되도록 α 조정
alpha = 4 * math.pi / (T_total**2)   # because θ = ½ α T_total^2 = 2π
a_t   = 1.0
alpha = a_t / R
frames_ctra_dt = []
for i in range(num_frames):
    t = i * dt
    θ = 0.5 * alpha * t**2      # θ(t) = ½ α t²
    x = round(R * math.cos(θ), 2)
    y = round(R * math.sin(θ), 2)
...
# ── 변수 정의 ──
num_frames = 20       # 총 프레임 수
dt         = 0.1       # 프레임 간 시간 간격 (초)
R0         = 5.0       # 시작 반지름 (m)
R1         = 10.0      # 최종 반지름 (m)
T_total    = num_frames * dt          # 한 바퀴 도는 총 시간 (초)
omega      = 2 * math.pi / T_total    # 일정 각속도 (rad/s)
k          = (R1 - R0) / T_total      # 반지름 변화율 (m/s)

# ── 프레임 데이터 생성 ──
frames_spiral = []
for i in range(num_frames):
    t     = i * dt
    r     = R0 + k * t            # 선형으로 늘어나는 반지름
    theta = omega * t             # 일정 각속도로 회전
    x, y  = round(r * math.cos(theta), 2), round(r * math.sin(theta), 2)
    frames_spiral.append([[x, y]])